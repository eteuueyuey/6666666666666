...
class StellarSweepBot {
  constructor() {
    this.server = new StellarSdk.Horizon.Server(PI_HORIZON_URL);
  ...
  schedulePrecisionTimer() {
    ...
    botState.precisionTimer = setTimeout(async () => {
      ...
      if (TRIGGER_OFFSET_MS > 0) {
        ...
        if (finalWait > 0 && finalWait < 1000) {
          console.log(`   Spin-waiting final ${finalWait}ms...`);
          const spinStart = Date.now();
          while (Date.now() - spinStart < finalWait) {
            // Busy wait
          }
        }
      }
