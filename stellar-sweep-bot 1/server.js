...
// Get claimable balances from DB
...
app.post('/api/bot/scan', async (req, res) => {
  try {
    const { scanAllWallets } = require('./scanner');
    ...
    <body>
      <div class="container">
        ...
        <div>
          ...
          <button class="btn" onclick="triggerScan()">Scan Wallets</button>
      ...
      <script>
        ...
        async function triggerScan() {
          try {
            ...
            const data = await response.json();
            alert('Scan started! Check logs for progress.');
          } catch (error) {
            alert('Error: ' + error.message);
          }
        }
